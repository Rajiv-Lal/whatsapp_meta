...
async function validate() {
  ...
  const pending = data.filter(r => String(r.status||'pending').trim() === 'pending').slice(0, 2000);
  log(`📋 Checking next ${pending.length} pending contacts`);
  ...
  client.on('ready', async () => {
    ...
    for (let i = 0; i < pending.length; i++) {
      ...
      if ((i+1) % 50 === 0) {
        ...
        log(`💾 ${i+1}/${pending.length} — ✅ ${valid} valid, ❌ ${invalid} not on WhatsApp`);
      }
